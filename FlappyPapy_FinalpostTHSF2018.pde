...
boolean score = true;
float extra_score = 0;
boolean was_level2 = false;
boolean reset_extra_score = false;
...
void draw() {
  ...
  if ( debug == false ) { inByte = myPort.read(); }
  ...
  case 1:
    ...
    if ( score ) {
    if ( (p.diametre*p.pos.x)/10000000 + extra_score > scores[0] ) {
      scores[2] = scores[1];
      scores[1] = scores[0];
      scores[0] = p.diametre*p.pos.x/10000000 + extra_score; }
      
    else if ( (p.diametre*p.pos.x)/10000000 + extra_score > scores[1] ) {
      scores[2] = scores[1];
      scores[1] = p.diametre*p.pos.x/10000000 + extra_score; }
      
    else if ( (p.diametre*p.pos.x)/10000000 + extra_score > scores[2] ) {
      scores[2] = p.diametre*p.pos.x/10000000 + extra_score; }
    ...
    if ( ( p.diametre*p.pos.x / 10000000 ) < 1 ) {
      style = true;
      textSize(55);
      text("'POP !'", width/2-textWidth("POP !")/2, height/2 );
      text("Score :  " + ( ( p.diametre * p.pos.x / 10000000 ) + extra_score ) + " BTC", width/2-textWidth("score")/2, height/2 + 100 );
      reset_extra_score = true; }
    
    else {
          textSize(65);
          text("'POPOPOP !'", width/2-textWidth("POPOPOP !")/2, height/2);
          textSize(55);
          text("Score :  " + ( ( p.diametre*p.pos.x ) / 10000000 ) + " BTC", width/2-textWidth("score")/2, height/2 + 100);
          
          textSize(20);
          text("Bravo ! En tant que nouveau riche,\nnous vous offrons ce moment de détente.", width/2, height/2+200);
          style = false; 
          extra_score = p.diametre * p.pos.x / 10000000;
          was_level2 = true;
      }
  ...
  case 0: 
    ...
    if ( reset_extra_score == true ) {
      extra_score = 0;
      reset_extra_score = false; }
...
void drawFG() { // aucune idée de a quoi ça sert ?
  fill(0,0, 148);
...
void drawPipes() {
  for (int i = 0; i < NUMPIPES; i++) { 
    ...
    if ( p.pos.x > ps[i].pos.x && p.pos.x < ps[i].pos.x + ps[i].bWidth && p.pos.y > ps[i].pos.y && p.pos.y < ps[i].pos.y + ps[i].bHeight )
      // test de collisision, actuellement juste le centre de la boule, a voir en rajoutant p.diametre
...
void drawGUI() {
  ...
  textAlign(RIGHT);
  if ( style ) {
  text("Score: " , 123, height - 20);
  textAlign(LEFT);
  text( ( ( p.diametre/p.pos.x ) + extra_score ) +" BTC" , 123, height - 20); //p.score = p.diametre/p.pos.x a utiliser pour un hight score!
} }
...
class PipeSet {
  ...
  boolean scored;
  int graphisme = int( random( 256 ) );
  int color_random = int( random( 150 ) );
  color base_color, shadow_color;
...
PipeSet(float x, float y, float h) {
  ...
  void drawPipes() { // les dessins des obstacle ( les barre sont de la decos, les ligne sont les limites de la cave)
...
class Player {
  ...
  void inflate(int i){ // methode pour gonfler le ballon
    diametre+=i;
  }


  void draw(Camera c) { // le controle du mouvement
    drawBird();
    ...
    pos.x += vitesse; // vitesse de scrolling joueur
  }

  void drawBird() { // l'avatar !
    if ( style ) {
      fill(214, 189, 47,200);
      noStroke();
      ellipse(pos.x, pos.y, p.diametre, p.diametre); // juste un rond !
       stroke(255);
      strokeWeight(1);
      line(pos.x+p.diametre/2.61,pos.y+p.diametre/2.61,pos.x,pos.y);
      line(pos.x+p.diametre/2.61,pos.y+p.diametre/2.61,pos.x+10+p.diametre/2.61,pos.y+p.diametre/2.61);
      text(p.diametre,pos.x+10+p.diametre/2.61,pos.y+5+p.diametre/2.61); }
...
// Addition des fonctions par Simon :
void draw_the_amazing_bird( PVector pos, int[] bird_life ) {
...
int[] update_bird_life( int[] bird_life ) {
  if ( bird_life[1] == 0 ) {
...
void draw_a_sublime_house( PVector pos, int house_width, color base_color, color shadow_color, int graphisme ) {
...
void draw_a_window_please( PVector pos ) {
...
void draw_wires( PVector pos, int wire_width, int previous_y ) {
